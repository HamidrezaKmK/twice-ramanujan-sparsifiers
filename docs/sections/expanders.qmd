...
:::proof
  The key aspect of the  proof is around the fact that if we consider the rayleigh coefficient ratio of $L_H$ for two different vectors. It can be at max $\kappa$ due to sparsifier definition.
...
$$
Since quadratic form for L_H, lowest possible value n and highest possible value $n\kappa$ is only true foor vecotr orthogonal to all single constant vector. So transforming variables
$$
