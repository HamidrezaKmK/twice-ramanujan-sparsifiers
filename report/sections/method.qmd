...
$$\Phi_{u, l}(A) = \Phi_u(A) + \Phi_l(A) = Tr[(uI - A)^{-1}] + Tr[(A - l I)^{-1}]$$
...
$$\infty \gg \Phi^{\ell^{(0)}}(A^{(0)}) \ge \Phi^{\ell^{(1)}}(A^{(1)}) \ge ... \ge \Phi^{\ell^{(nd)}}(A^{(nd)})$$
...
Similar to @eqn-l-potential, if we negate $s$ and $A$ then the upper-bound potential will act similarly to the lower-bound potential. Therefore, we can write the following:
$$s \le U_A(v) = \frac{\Delta}{v^T \left((u' I - A)^{-2} - \Delta (u' I - A)^{-1}\right)v}, ~~ \Delta = \Phi^{u}(A) - \Phi^{u'}(A)$$
